...
$padre="";
	...
	if (mysqli_connect_errno())  {
		...
		$tmpcat="SELECT idpadre from categorias where id=$clave";
		$resulcat = mysqli_query($con,$tmpcat);
		while($rowcat = mysqli_fetch_array($resulcat)){
				$padre=$rowcat['idpadre'];
				echo "padre=".$padre;
		}
		//echo "padre=".$padre;
		...
		if (strcmp($padre,"0")==0) if (strlen($clave)>0) $tmptuits.=" and idcategoria in (select idpadre from categorias where idpadre=$clave)" ;
		if (strcmp($padre,"0")!=0) if (strlen($clave)>0) $tmptuits.=" and idcategoria=$clave" ;
